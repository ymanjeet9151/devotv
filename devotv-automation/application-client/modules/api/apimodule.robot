...
| Library | ../application-client/resources/payloadAdminPublisher.py
| Library | ../application-client/resources/payload.py
...
| | [Documentation] | User expected to set the Variables for admin advertiser contract product offerasset
| | ${admin_advertiser_contract_product_offerasset_Image} | User get the payload from datafiles | Email-Offer-Image-create-admin-advertiser-contract-product-offerasset.json
...
| | [Arguments] | ${uri} | ${response_status}
| | User Initiate GET call and verify Status Response | ${uri} | ${admad_datafile}get-all-existing-user-advertiser.json | ${response_status}
...
| | [Arguments] | ${uri} | ${Expected_status_code}
| | User Initiate POST call and verify Status Response | ${uri} | ${admad_datafile}add-new-and-brand-advertiser.json | ${Expected_status_code} | ${body}

| GET existing users by ID for advertiser
| | [Documentation] | User Expected to get the existing data by ID
| | [Arguments] | ${uri} | ${Expected_status_code}
| | User Initiate GET call and verify Status Response | ${uri} | ${admad_datafile}get-all-existing-user-by-ID-advertiser.json | ${Expected_status_code}
| | ${resp_data} | Output | response body
...
| User Deletes an Existing data for advertiser
| | [Documentation] | User expects to Delete a data by providing the brand's ID for advertiser
| | [Arguments] | ${uri} | ${Expected_status_code}
| | User Initiate DELETE call and verify Status Response | ${uri} | ${admad_datafile}delete-existing-data-advertiser.json | ${Expected_status_code}
| | ${resp_data} | Output | response body
...
| User Adds the new user
| | [Documentation] | User Expected to Add new user data
| | [Arguments] | ${uri} | ${Expected_status_code} | ${content}
| | User Initiate Post call and verify Status Response | ${uri} | ${admad_datafile}adds-new-user.json | ${Expected_status_code} | ${body}
| | ${resp_data} | Output | response body
| | Should Be Equal | ${resp_data}[${content}] | ${data_variable}[${content}]

| GET all existing user for Admin
| | [Documentation] | User Expected to
| | [Arguments] | ${uri} | ${Expected_status_code}
| | User Initiate GET call and verify Status Response | ${uri} | ${admad_datafile}get-all-existing-user-admin.json | ${Expected_status_code}
| | ${resp_data} | Output
| | Should be Equal | ${resp_data}[response][body][advertisers][0][advertiser_id] | ${data_variable}[advertiser_id]
| | Should be Equal | ${resp_data}[response][body][advertisers][0][advertiser_name] | ${data_variable}[advertiser_name]
| | Should be Equal | ${resp_data}[response][body][advertisers][0][site_url] | ${data_variable}[site_url]
| | Should be Equal | ${resp_data}[response][body][advertisers][0][client_business_type] | ${data_variable}[client_business_type]

| GET all existing users status for Admin
| | [Documentation] | User Expected to
| | [Arguments] | ${uri} | ${Expected_status_code}
| | User Initiate GET call and verify Status Response | ${uri} | ${admad_datafile}get-all-existing-users-status-admin.json | ${Expected_status_code}
| | ${resp_data} | Output
| | Output | response status
| | Log | ${resp_data}[response][body][0][advertiser_id]
| | Should Be Equal | ${resp_data}[response][body][0][advertiser_id] | ${data_variable}[advertiser_id]
| | Should Be Equal | ${resp_data}[response][body][0][advertiser_name] | ${data_variable}[advertiser_name]
| | Should Be Equal | ${resp_data}[response][body][0][site_url] | ${data_variable}[site_url]

| GET all admin users as Admin
| | [Documentation] | User Expected to get all the brands
| | [Arguments] | ${uri} | ${Expected_status_code}
| | User Initiate GET call and verify Status Response | ${uri} | ${admad_datafile}Get-all-admin-user.json | ${Expected_status_code}
| | ${resp_data} | Output | response body
| | Log | ${resp_data}[0]
| | Log | ${data_variable}
| | Should Be Equal | ${resp_data}[0][id] | ${data_variable}[id]
| | Should Be Equal | ${resp_data}[0][user_email] | ${data_variable}[user_email]
| | Should Be Equal | ${resp_data}[0][user_role_id] | ${data_variable}[user_role_id]
| | Should Be Equal | ${resp_data}[0][user_first_name] | ${data_variable}[user_first_name]
| | Should Be Equal | ${resp_data}[0][user_last_name] | ${data_variable}[user_last_name]
| | Should Be Equal | ${resp_data}[0][status] | ${data_variable}[status]
...
| User GET all the publishers data as Admin
| | [Documentation] | User expected to Update the Existing data by Userid as Admin
| | [Arguments] | ${uri} | ${Expected_status_code}
| | User Initiate GET call and verify Status Response | ${uri} | ${admad_datafile}get-all-publishers-data-as-admin.json | ${Expected_status_code}
| | ${resp_data} | Output | response body
| | Log | ${data_variable}
| | Log | ${resp_data}[publishers][0][publisher_id]
| | Should Be Equal | ${resp_data}[publishers][0][publisher_id] | ${data_variable}[publisher_id]
| | Should Be Equal | ${resp_data}[publishers][0][contract_id] | ${data_variable}[contract_id]
| | Should Be Equal | ${resp_data}[publishers][0][publisher_name] | ${data_variable}[publisher_name]
| | Should Be Equal | ${resp_data}[publishers][0][site_url] | ${data_variable}[site_url]
| | Should Be Equal | ${resp_data}[publishers][0][client_business_type] | ${data_variable}[client_business_type]
